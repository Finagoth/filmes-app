...
export default function BuscaAtores() {
          ...
          {atores.map(ator => {
            ...
            return (
              <Link key={ator.id} href={`/atores/${ator.id}`} className="group block">
                <div className="rounded-xl overflow-hidden border shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 flex flex-col"
                  ...
                  <div className="p-3 flex flex-col justify-between" style={{ minHeight: '62px' }}>
                    <h2 className="font-semibold text-sm leading-tight line-clamp-2"
                      style={{ color: 'var(--text-primary)' }}>{ator.name}</h2>
